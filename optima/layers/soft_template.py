...
class SoftTemplate(Template):
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        ...
        batch['input_ids'] = None  ## TODO Why do this? Encoder recognizes input_ids first:
...
class MySoftTemplate(SoftTemplate):
    def __init__(self,
                 plm: PreTrainedModel,
                 tokenizer: PreTrainedTokenizer,
                 shared_num_tokens: int=0,
                 unshared_num_tokens: int=100,
                 unshared_soft_embeds: Optional[torch.nn.Parameter] = None,
                 shared_soft_embeds: Optional[torch.nn.Parameter] = None,
                 initialize_from_vocab: Optional[bool] = True):
        super().__init__(
            model=plm,
            tokenizer=tokenizer,
            num_tokens=unshared_num_tokens,
            initialize_from_vocab=initialize_from_vocab
        )
        self.shared_num_tokens = shared_num_tokens
        self.unshared_num_tokens = unshared_num_tokens
        self.soft_num_tokens = shared_num_tokens + unshared_num_tokens
        self.soft_embeds = None

        if shared_soft_embeds is not None:
            self.shared_soft_embeds = shared_soft_embeds
            self.shared_num_tokens = len(shared_soft_embeds)
        else:
            raise NotImplementedError

        if unshared_soft_embeds is not None:
            self.unshared_soft_embeds = unshared_soft_embeds
            self.unshared_num_tokens = len(unshared_soft_embeds)
        else:
            raise NotImplementedError

    def input_ids_to_embeds(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        inputs_embeds = self.raw_embedding(batch['input_ids'])
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        ...
        if self.soft_num_tokens > 0:
            domain_soft_embeds = self.unshared_soft_embeds.repeat(batch_size, 1, 1)
            shared_soft_embeds = self.shared_soft_embeds.repeat(batch_size, 1, 1)
            inputs_embeds = torch.cat([shared_soft_embeds, domain_soft_embeds, inputs_embeds], 1)

        batch['inputs_embeds'] = inputs_embeds

        if 'attention_mask' in batch and self.soft_num_tokens > 0:
            am = batch['attention_mask']
            batch['attention_mask'] = torch.cat([torch.ones((batch_size, self.soft_num_tokens), dtype=am.dtype, device=am.device), am], dim=-1)
        return batch
