...
class MySoftTemplate(SoftTemplate):
    def __init__(self,
                 plm: PreTrainedModel,
                 tokenizer: PreTrainedTokenizer,
                 domain_num_tokens: int=100,
                 domain_soft_embeds: Optional[torch.nn.Parameter] = None,
                 initialize_from_vocab: Optional[bool] = True):
        super().__init__(
            model=plm,
            tokenizer=tokenizer,
            num_tokens=domain_num_tokens,
            initialize_from_vocab=initialize_from_vocab
        )
        self.domain_num_tokens = domain_num_tokens
        self.soft_num_tokens = domain_num_tokens
        self.soft_embeds = None


        if domain_soft_embeds is not None:
            self.domain_soft_embeds = domain_soft_embeds
            self.domain_num_tokens = len(domain_soft_embeds)
        else:
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        ...
        if self.soft_num_tokens > 0:
            domain_soft_embeds = self.domain_soft_embeds.repeat(batch_size, 1, 1)
            inputs_embeds = torch.cat([domain_soft_embeds, inputs_embeds], 1)
