...
def get_label(task, line):
    if task in ["MNLI", "MRPC", "MRPC_pp",  "QNLI", "QQP", "RTE", "SNLI", "SST-2", "GLUE-SST-2", "STS-B", "WNLI", "CoLA"]:
        # GLUE style
        ...
        if task == 'CoLA':
            ...
            return line[-1]
        elif task == 'MRPC' or task == 'MRPC_pp':
            return line[0]
        elif task == 'QNLI':
            return line[-1]
        elif task == 'QQP':
            ...
            return line[-1]
        elif task == 'SST-2' or task == 'GLUE-SST-2':
            return line[-1]
        elif task == 'STS-B':
...
def load_datasets(data_dir, tasks):
    datasets = {}
    for task in tasks:
        if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "GLUE-SST-2", "STS-B", "WNLI", "CoLA"]:
            # GLUE style (tsv)
            dataset = {}
        ...
        if task == "MRPC":
            ...
            filename = os.path.join(dirname, f"msr_paraphrase_train.txt")
            with open(filename, "r") as f:
                lines = f.readlines()
            dataset["train"] = lines
            filename = os.path.join(dirname, f"msr_paraphrase_test.txt")
            with open(filename, "r") as f:
                lines = f.readlines()
            dataset["dev"] = lines
            datasets["MRPC_pp"] = dataset
...
def split_header(task, lines):
    ...
    if task in ["CoLA"]:
        return [], lines
    elif task in ["MNLI", "MRPC", "MRPC_pp", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "GLUE-SST-2", "STS-B", "WNLI"]:
...
def main():
    ...
    parser.add_argument("--task", type=str, nargs="+", 
        default=['SST-2', 'GLUE-SST-2', 'sst-5', 'mr', 'cr', 'mpqa', 'subj', 'trec', 'CoLA', 'MRPC', 'QQP', 'STS-B', 'MNLI', 'SNLI', 'QNLI', 'RTE'],
        # default=['cr'],
        help="Task names")
    parser.add_argument("--seed", type=int, nargs="+", 
        default=[100, 13, 21, 42, 87, 36, 52, 68, 78, 93, 234, 63, 527, 628, 29, 15],
        help="Random seeds")
    ...
    parser.add_argument("--mode", type=str, default='128-shot', choices=['k-shot', 'k-shot-10x'], help="k-shot or k-shot-10x (10x dev set)")
